...
def test_decision_builder_exports_model_and_hammer_confidence_separately():
    with tempfile.TemporaryDirectory() as directory:
        root = Path(directory)
        mlb_path = root / "mlb.json"
        first5_path = root / "first5.json"
                    ...
                    "games": [
                        {
                            ...
                            "model": {
                                ...
                                "model_win_strength": 59.0,
                                "model_probability": 59.0,
                                "model_confidence": 80.0,
                                "component_scores": {
                                    "selected": {
                                        "offense": 64.0,
                                        "starting_pitching": 70.0,
                                        "bullpen": 58.0,
                                    },
                                    "opponent": {},
                                },
    ...
    assert decision["recommendation"] == "✅ STRONG PLAY"
    assert decision["model_win_strength"] == decision["model_probability"]
    assert decision["model_confidence"] == 80.0
    assert decision["hammer_confidence"] != decision["model_confidence"]
    assert decision["confidence"] == decision["hammer_confidence"]
